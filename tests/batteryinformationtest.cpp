...
TEST_F(BatteryInformationTest, ReplNo_RechNo_ManNo_ExpNo_CapNo_LowNo_CritNo_ChemNo_VoltNo_AggGrpNo_ResAll)
{
    ...
    float abs_error;
    abs_error = std::pow(10, result->batteryDesignedCapacity().exponent());
    EXPECT_NEAR(0.0, result->batteryDesignedCapacity().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryLowEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryCriticalEnergy().toFloat(), abs_error);
    EXPECT_EQ(BatteryChemistryEnum::Unknown, result->batteryChemistry());
    abs_error = std::pow(10, result->nominalVoltage().exponent());
    EXPECT_NEAR(0.0, result->nominalVoltage().toFloat(), abs_error);
...
TEST_F(BatteryInformationTest, ReplYes_RechNo_ManDate_ExpNo_CapMax_LowNo_CritNormal_ChemNo_Volt0_AggGrpNo_ResAll)
{
    ...
    float abs_error;
    abs_error = std::pow(10, result->batteryDesignedCapacity().exponent());
    EXPECT_NEAR(20470000000.0, result->batteryDesignedCapacity().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryLowEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(17.0, result->batteryCriticalEnergy().toFloat(), abs_error);
    EXPECT_EQ(BatteryChemistryEnum::Unknown, result->batteryChemistry());
    abs_error = std::pow(10, result->nominalVoltage().exponent());
    EXPECT_NEAR(0.0, result->nominalVoltage().toFloat(), abs_error);
    EXPECT_EQ(0, result->batteryAggregationGroup());
...
TEST_F(BatteryInformationTest, ReplNo_RechYes_ManNo_ExpDate_CapNo_LowMax_CritNo_ChemUnknown_VoltNo_AggGrpNotIn_ResAll)
{
    ...
    EXPECT_NEAR(0.0, result->batteryDesignedCapacity().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryLowEnergy().exponent());
    EXPECT_NEAR(20470000000.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryCriticalEnergy().toFloat(), abs_error);
    EXPECT_EQ(BatteryChemistryEnum::Unknown, result->batteryChemistry());
    abs_error = std::pow(10, result->nominalVoltage().exponent());
...
TEST_F(BatteryInformationTest, ReplNo_RechNo_ManDate_ExpDate_CapNo_LowNo_CritMax_ChemAlkaline_VoltNo_AggGrpNo_ResAll)
{
    ...
    EXPECT_NEAR(0.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(20470000000.0, result->batteryCriticalEnergy().toFloat(), abs_error);
...
TEST_F(BatteryInformationTest, ReplYes_RechYes_ManNo_ExpNo_CapNormal_Low0_Crit0_ChemLeadAcid_VoltNormal_AggGrpNormal_ResNone)
{
    ...
    float abs_error;
    abs_error = std::pow(10, result->batteryDesignedCapacity().exponent());
    EXPECT_NEAR(17.0, result->batteryDesignedCapacity().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryLowEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(0.0, result->batteryCriticalEnergy().toFloat(), abs_error);
    ...
    abs_error = std::pow(10, result->nominalVoltage().exponent());
    EXPECT_NEAR(17.0, result->nominalVoltage().toFloat(), abs_error);
...
TEST_F(BatteryInformationTest, ReplYes_RechYes_ManDate_ExpNo_Cap0_LowNormal_CritMax_ChemLiFeS2_VoltMax_AggGrpReserved_ResNone)
{
    ...
    EXPECT_NEAR(0.0, result->batteryDesignedCapacity().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryLowEnergy().exponent());
    EXPECT_NEAR(17.0, result->batteryLowEnergy().toFloat(), abs_error);
    abs_error = std::pow(10, result->batteryCriticalEnergy().exponent());
    EXPECT_NEAR(20470000000.0, result->batteryCriticalEnergy().toFloat(), abs_error);
    EXPECT_EQ(BatteryChemistryEnum::LiFeS2, result->batteryChemistry());
    abs_error = std::pow(10, result->nominalVoltage().exponent());
    EXPECT_NEAR(20470000000.0, result->nominalVoltage().toFloat(), abs_error);
